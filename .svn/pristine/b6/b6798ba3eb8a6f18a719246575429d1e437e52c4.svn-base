...
class Model {
     ...
     */
     protected function _facade($data) {
        ...
        if(!empty($this->fields)) {
			...
			$privateField = array_keys( D()->privateField);
            foreach ($data as $key=>$val){
                if(!in_array($key,$fields,true)){
                    unset($data[$key]);
				}elseif(is_scalar($val)) {
					// 字段权限检查
					if(!empty($privateField) && in_array($key, $privateField, true) ) {
						unset($data[$key]);
					}
					// 字段类型检查 和 强制转换
	...
	public function query($sql,$parse=false) {
		...
		if( ENV != 'production' && $this->db->masterOnly !== true)  {
			$str = substr(strtolower(trim($sql)), 0, 6);
			if($str != "select") {
				throw new \Exception('DML must be use func master() on query.');
			#	return false;
			}
		}
	...
	public function db($linkNum='',$config='',$force=false) {
		...
		static $_db = array();
		if($linkNum !== 0) {
			$_GET['isChangeDbModel'] = true;
		}else {
			if($_GET["isChangeDbModel"] == true) {
				$_GET["closeChangeModel"] = true;
    ...
    */
    ...
    public function parseWhereCondition($where){
        
        if(is_array($where) && !is_object($where)){
            $where = $this->db->listWhere($where);
        }
