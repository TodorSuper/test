...
class Db {
    ...
    // 当前事务状态
    public $transStatus    = 'close';
    // 是否已经连接数据库
    protected $connected  = false;
    // 是否其他库切换模式
    public $isChangeDbModel  = false;
    // 是否关闭其他库切换模式
    public $closeChangeModel  = false;
    // 数据库连接参数配置
    protected $config     = '';
    // 数据库表达式
    ...
    protected function multiConnect($master=false) {
		...
		if($this->masterOnly == true) {
			$master = true;
		}
		if($_GET["closeChangeModel"] === true) {
			$_GET["isChangeDbModel"] = false;
			$_GET["closeChangeModel"] = false;
			$_config = array();
		}
		if(empty($_config)) {
            // 缓存分布式数据库配置解析
	...
	protected function debug() {
        ...
        if (C('DB_SQL_LOG')) {
			G('queryEndTime');
			$exeTime_s = G('queryStartTime','queryEndTime',6);
			$exeTime_ms = $exeTime_s*1000;
			$result = $this->speed($exeTime_ms);
			$str = $this->queryStr.' [ RunTime: '.$exeTime_s.'s '.$exeTime_ms.'ms ]'."  [ speed: $result ]";
			if(ENV == 'production') {
				DG($str, SQL_DG_OBJECT);
			}else {
				DG($str, SQL_DG_OBJECT);
				trace($str,'','SQL');
			}
			...
			$exeTime_s = G('queryStartTime','queryEndTime',6);
			$exeTime_ms = $exeTime_s*1000;
			$str = $this->queryStr.' [ RunTime: '.$exeTime_s.'s '.$exeTime_ms.'ms ]';
			DG($str, SQL_DG_OBJECT);
		}
	}
	...
	private function speed($time) {
		if($time <= 1) {
			return "火箭";
		}elseif($time > 1 && $time <= 20 ) {
			return "飞机";
		}elseif($time > 20 && $time <= 100 ) {
			return "汽车";
		}elseif($time > 100 && $time <= 500 ) {
			return "摩托车";
		}elseif($time > 1000 && $time <= 2000 ) {
			return "步行";
		}elseif($time > 2000 && $time <= 5000 ) {
			return "蜗牛";
		}else{
			return "等着罚款吧";
		}	
	...
	protected function parseSet($data) {
        foreach ($data as $key=>$val){
			if(is_array($val) && 'exp' == $val[0]){
                ...
                $set[]  =   $this->parseKey($key).'='.$key.$val[0].$val[1];
            ...
            foreach ($where as $key=>$val){
                ...
                if(0===strpos($key,'_')) {
                    ...
                    if(!preg_match('/^[A-Z_\|\&\-.a-z0-9\(\)\,]+$/',trim($key))){
						throw new \Exception('the where condition is not safe -  _EXPRESS_ERROR_'.':'.$key);
                    }
                    // 多条件支持
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT)$/i',$val[0])) { // 比较运算
					...
					throw new \Exception('the where condition is not safe -  _EXPRESS_ERROR_'.':'.$val[0]);
                }
            }else {
                $count = count($val);
