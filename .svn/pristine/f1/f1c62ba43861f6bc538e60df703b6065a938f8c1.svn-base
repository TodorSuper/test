...
class Umeng {
	...
	public function __construct($plateform = BOSS,$device=ANDROID) {
                $plateform_set = C($plateform)[$device];
		$this->appkey = $plateform_set['APP_KEY'];
		$this->appMasterSecret =$plateform_set['APP_MASTER_SECRET'];
		$this->timestamp = strval(time());
	}

	public function sendAndroidBroadcast() {  //广播
		try {
         ...
         * @return boolean
         */
	public function sendAndroidUnicast($device_tokens,$ticker,$title,$text,$play_sound = true,$after_open='go_app',$activity = '') {  //单播
		try {
			...
			$unicast->setPredefinedKeyValue("play_sound",       $play_sound);
                        if($after_open == 'go_activity'){
                            $unicast->setPredefinedKeyValue("activity",       $activity);
                        }
			// Set 'production_mode' to 'false' if it's a test device. 
			// For how to register a test device, please see the developer doc.
			$unicast->setPredefinedKeyValue("production_mode", (bool)!APP_DEBUG);  //app_debug 关闭的状态下为正式环境  开启时为测试环境
			// Set extra fields
...
//			print("Caught exception: " . $e->getMessage());
	...
	public function sendAndroidFilecast() {  //文件播
		try {
	...
	public function sendAndroidGroupcast() {  //组播
		try { 
			/* 
	...
	public function sendIOSBroadcast() {  //广播
		try {
	...
	public function sendIOSUnicast($device_tokens,$ticker,$play_sound) {   //单播
		try {
			...
			// Set 'production_mode' to 'true' if your app is under production mode
			$unicast->setPredefinedKeyValue("production_mode", (bool)!APP_DEBUG);
			// Set customized fields
