...
class AccountSync extends Base
{
    ...
    public function syncBankList($params=array())
    {
		...
		$where['account_bank'] = PAY_METHOD_REMIT_CMBC;
    ...
    private function runInsertUpdate($bankRes,$startNo) {
        ...
        foreach($bankRes['response']['response']['list'] as $b_key=>$b_val) {
            ...
            $data[] = array(
                ...
                'bank_type' => $bank_type,
				'account_bank' => PAY_METHOD_REMIT_CMBC,
            );
            $startNo++;
        }
        if($data) {
    ...
    public function QryXfer() {
        ...
        $account_codes = D('FcPaymentAccount')->where('fc_check_status="OK" AND (fc_pay_status="" OR (fc_pay_status="FAIL" AND pay_times<3))  AND bank_type="CMBC"')->Field('fc_code,account_code')->limit(11)->select();
        if(empty($account_codes)) {
            return $this->res(null,8051);
        }
    ...
    public function QryXferSplit() {
        ...
        $payment_codes = D('FcOrderPayment')->where('status=1 AND ( `fc_check_status` <> "OK" OR `fc_pay_status` <> "OK") AND bank_type="CMBC"')->Field('fc_code,sc_code')->select();
