...
class Mysql extends Db{
	...
	public function startTrans() {
		DG('startTrans', SUB_DG_OBJECT);
		$this->initConnect(true);
		$this->transStatus = 'start';
        if ( !$this->_linkID ) return false;
        //数据rollback 支持
        if ($this->transTimes == 0) {
			mysql_query('START TRANSACTION', $this->_linkID);
			$this->_masterLinkID = $this->_linkID;
        }
        $this->transTimes++;
        return ;
    }
    ...
    public function commit() {
		DG('commit', SUB_DG_OBJECT);
		$this->masterOnly = true;
		$this->transStatus = 'close';
        if ($this->transTimes > 0) {
            $result = mysql_query('COMMIT', $this->_masterLinkID);
            $this->transTimes = 0;
            if(!$result){
                $this->error();
                return false;
	...
	public function rollback() {
		DG('rollback', SUB_DG_OBJECT);
        DG($this->error(733), SUB_DG_OBJECT);
		$this->masterOnly = true;
		$this->transStatus = 'close';
        if ($this->transTimes > 0) {
            $result = mysql_query('ROLLBACK', $this->_masterLinkID);
            $this->transTimes = 0;
            if(!$result){
                $this->error();
                return false;
     ...
     * @return string
     */
    public function error($throw = true) {
